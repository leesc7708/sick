...
import { PHRASE_GROUPS, PHRASES, Phrase, PhraseGroupId } from '../data/emergencyPhrases';
import { speakKorean, stopSpeaking } from '../services/speak';
...
// 화면 UI 문구도 7개 언어(자국어). 데이터(문장·그룹명)는 emergencyPhrases.ts에서 자국어로.
const UI: Record<string, Record<Lang, string>> = {
  ...
  soundOff: { ko: '소리가 안 나면 위 한국어 글자를 보여주세요.', en: 'If there is no sound, show the Korean text above.', zh: '如果没有声音，请出示上面的韩语文字。', ja: '音が出ない場合は、上の韓国語の文字を見せてください。', vi: 'Nếu không có âm thanh, hãy đưa chữ Hàn ở trên.', th: 'ถ้าไม่มีเสียง ให้แสดงข้อความภาษาเกาหลีด้านบน', es: 'Si no hay sonido, muestre el texto coreano de arriba.' },
};
...
export function PhrasebookScreen({ navigation }: Props) {
  ...
  const [active, setActive] = useState<{ p: Phrase; noVoice: boolean } | null>(null);

  const play = (p: Phrase) => {
    const r = speakKorean(p.ko);
    setActive({ p, noVoice: r !== 'ok' });
  };

  const report = (p: Phrase) => {
    // MVP: 로컬 확인만(백엔드 없음). 오역 사후 수정 채널.
    Alert.alert(t('report'), `"${p.text[lang]}"`, [{ text: 'OK', onPress: () => Alert.alert(t('reported')) }]);
  };
  ...
  return (
    <View style={styles.wrap}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {/* 재생 중 문장: 큰 한국어 자막 (소리+화면 이중 채널) */}
        {active && (
          <View style={[styles.activeCard, shadow.card]}>
            <Text style={[typography.small, { color: colors.primary, marginBottom: 4 }]}>{t('showThis')}</Text>
            <Text style={styles.activeKo}>{active.p.ko}</Text>
            <Text style={[typography.caption, { color: colors.textMuted, marginTop: 6 }]}>{active.p.text[lang]}</Text>
            {active.noVoice && <Text style={[typography.small, { color: colors.warning, marginTop: 6 }]}>🔇 {t('soundOff')}</Text>}
            <View style={styles.activeBtns}>
              <PrimaryButton title={t('replay')} icon="🔎" variant="primary" size="sm" onPress={() => play(active.p)} />
            </View>
          </View>
        )}
        ...
        {group && groupObj && (
          <>
            ...
            {list.map((p) => (
              <Pressable key={p.id} onPress={() => play(p)} style={[styles.phrase, shadow.card, active?.p.id === p.id && { borderColor: colors.primary, borderWidth: 1.5 }]}>
                <View style={{ flex: 1 }}>
                  ...
                  <Pressable onPress={() => report(p)} hitSlop={8}><Text style={styles.reportTxt}>{UI.report[lang]}</Text></Pressable>
                </View>
...
const styles = StyleSheet.create({
  ...
  activeKo: { fontSize: 26, fontWeight: '800', color: colors.text, lineHeight: 34 },
  activeBtns: { flexDirection: 'row', marginTop: spacing.md },
});
