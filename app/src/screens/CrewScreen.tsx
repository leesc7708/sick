  ...
  return (
    <View style={styles.wrap}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* 오늘 그룹 만들기 */}
        <Text style={styles.h}>오늘 그룹 만들기</Text>
        <TextInput value={label} onChangeText={setLabel} placeholder="작업 그룹 이름 (예: A현장 3층 배관)" placeholderTextColor={colors.g400} style={styles.input} />
        <TextInput value={endTime} onChangeText={setEndTime} placeholder="작업 종료 예정 시각 (예: 오늘 22:00 / 내일 06:00)" placeholderTextColor={colors.g400} style={styles.input} />
        <View style={{ marginTop: spacing.sm }}><PrimaryButton title="그룹 만들기" icon="＋" onPress={make} /></View>

        {/* 오늘 내 그룹 */}
        <Text style={styles.h}>오늘 내 그룹 ({crews.length})</Text>
        {crews.length === 0 && <Text style={[typography.caption, { color: colors.textMuted }]}>아직 없음</Text>}
        {crews.map((c) => (
          <Pressable key={c.id} onPress={() => setSel(c)} style={[styles.crewCard, shadow.card, sel?.id === c.id && { borderColor: colors.primary, borderWidth: 1.5 }]}>
        ...
        {sel && (
          <>
            <Text style={styles.h}>[{sel.label}] 워커 추가</Text>
            <View style={{ flexDirection: 'row' }}>
              <TextInput value={uname} onChangeText={setUname} placeholder="워커 아이디" placeholderTextColor={colors.g400} autoCapitalize="none" style={[styles.input, { flex: 1, marginTop: 0 }]} />
              <View style={{ width: 8 }} />
              <PrimaryButton title="추가" size="sm" onPress={add} style={{ width: 80 }} />
            </View>
            <Text style={[styles.h, { fontSize: 14 }]}>멤버 {members.length}명</Text>
            {members.map((m) => (
              <View key={m.workerUid} style={[styles.memberRow, shadow.card]}>
                <View style={{ flex: 1 }}>
