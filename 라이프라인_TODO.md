...
**권역외상센터 단독 고착 → 2겹 방어 도입(2026-08-26 21시경) — 종결**

- **증상**: 429가 풀린 뒤에도 외상센터만 약 3시간 502 고착. 나머지 5종은 정상(병의원 19,340 / 약국 5,442 / AED 12,280 / 중증 54 / 병상 55).
- **원인 규명**(가설 두 개를 실측으로 차례로 기각):
  1. "인스턴스가 죽은 연결을 물고 있다" → **functions 재배포해도 그대로 502. 기각.**
  2. "상류가 느리다" → 이 PC에서 **같은 키·같은 파라미터·같은 accept 헤더로 122ms, 5,606바이트 정상**. 기각.
  - 함수 로그: `AbortError`, `cause.code` 없음 = 소켓 에러가 아니라 **응답이 그냥 안 옴(행)**. `stage1` 유무·Hosting 우회 무관하게 16.1초(8초×2회) 고정.
  - 같은 호스트·같은 서비스의 다른 오퍼레이션은 Cloud Run에서 정상 ⇒ 네트워크 경로 문제도 아님.
  - ⇒ **공공데이터포털이 Cloud Run 송신 IP에 대해 그 오퍼레이션만 막은 것**으로 판단.
- ⚠️ **자책 사항**: 17:36·17:40에는 20건 정상이었다. 17:50부터 감시(6종 중 외상센터를 **맨 앞에서** 5분마다)와 수동 3회 재시도 루프를 반복하면서 고착됐다. **장애 조사용 폴링이 그 자체로 차단을 유발했다.** 앞서 같은 교훈을 기록해 놓고 외상센터에는 적용하지 않았다.
- **조치 — 2겹으로 막는다**:
  1. **서버 캐시**(`functions/index.js`): 성공 응답을 **6시간** 인스턴스 메모리에 보관. 신선하면 상류를 아예 안 부른다. 상류가 죽으면 **만료된 캐시라도** `stale:true`·`cachedAt`과 함께 내려준다.
  2. **앱 동봉 정적 스냅샷**(`src/data/traumaCenters.ts`, 20곳, `TRAUMA_SNAPSHOT_DATE='2026-08-26'`): 콜드스타트라 서버 캐시도 비었을 때의 최후 방어선. `fetchTrauma()`가 실패하면 이걸 반환하고 `status:'stale'`로 알린다.
  - `EgenResult`에 `'stale'` 추가. 화면은 stale일 때 **"실시간 조회 실패 — 저장된 목록입니다 · 2026-08-26 기준"** 배지를 띄운다. **오래된 정보를 최신인 척하지 않는다**(①과 같은 원칙).
  - `trauma_stale`·`trauma_stale_asof` **7개 언어** 추가.
- **검증**: `tsc --noEmit` 통과 / `check-i18n` **381키 × 7개 언어 일치**, `t()` 327키 정의 / 번들·라이브 양쪽에 스냅샷 데이터(`A2100040` 등)와 배지 문구 실물 확인 / `audio-ko` 회귀 없음.
- **배포 완료**: functions 7종 + `egenTrauma` + hosting. **최종 6/6 전부 정상**이며 외상센터는 `cached:true`로 응답(상류 미호출). 결과적으로 차단도 해소됐지만, 2겹 방어는 다음 번을 위해 남는다.
- 교훈: **거의 안 바뀌는 목록을 매번 실시간 조회하면 안 된다.** 전국 20곳짜리 목록 때문에 상류에 차단당해 화면이 3시간 비었다. 그리고 **응급 앱에서 어떤 화면은 비면 안 된다** — 어디로 이송할지 정하는 목록이라면 오래된 정보라도 기준일을 밝히고 보여주는 게 맞다.
