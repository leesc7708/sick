...
**복구 확인 중 발견한 별건 결함(2026-08-26 17:40경) — 재시도 안전장치가 절반에만 적용돼 있었다**

- 속도제한이 풀린 뒤 전수검사: 외상센터 20건 · 중증수용 54건 · **AED 12,280건** · 약국 30건 · 응급실병상 20건 정상. **병·의원(egenHospitals)만 4회 연속 502.**
- 응답 본문이 `{"ok":false,"error":"fetch failed"}` — **우리 함수는 실행됐고 상류로 나가는 연결만 실패**. 같은 키로 이 PC에서 같은 오퍼레이션을 부르면 **601ms에 정상 응답**이라 E-Gen 장애도 키 문제도 아니다.
- 원인: 08-14에 만든 `fetchUpstream()`(타임아웃 8초 + 1회 재시도 + 원인 로깅)이 **`egenBeds`·`egenTrauma` 2종에만** 붙어 있었다. `egenSevere`·`egenAed`·`egenHospitals`·`egenPharmacy` 4종은 맨 `fetch()`라 **상류가 한 번만 삐끗해도 즉시 502**였고, 실패 원인도 로그에 안 남았다.
- 조치: **6종 전부 `fetchUpstream` + `upstreamErrorPayload` 경유로 통일.** `node --check` 통과.
- ⏳ **functions 미배포** — 사용자 확인 후 `firebase deploy --only functions` 필요(`.env` 있어야 함, 규칙 #1).
- 교훈: **안전장치를 한 곳에 만들었으면 같은 패턴 전부에 적용했는지 그 자리에서 확인할 것.** 절반만 고치면 나머지 절반이 다음 장애가 된다. AED는 집계 키를 `name`으로 잘못 잡아 "0건"으로 오판할 뻔했다 — **엔드포인트마다 반환 필드가 다르다**(AED는 `place`).
